...
use crate::{
    context::ValidatorContext, diagnostics::Diagnostic, fs::FileSystem,
...
impl AgentValidator {
    ...
    fn find_project_root<'a, F: FileSystem + ?Sized>(path: &'a Path, fs: &F) -> Option<&'a Path> {
    ...
    fn skill_exists<F: FileSystem + ?Sized>(project_root: &Path, skill_name: &str, fs: &F) -> bool {
    ...
    fn validate(&self, path: &Path, content: &str, ctx: &ValidatorContext) -> Vec<Diagnostic> {
        ...
        if ctx.is_rule_enabled("CC-AG-005") {
            if let Some(skills) = &schema.skills {
                if let Some(project_root) = Self::find_project_root(path, ctx.fs) {
                    for skill_name in skills {
                        if !Self::skill_exists(project_root, skill_name, ctx.fs) {
                            diagnostics.push(
                                Diagnostic::error(
                                    path.to_path_buf(),
                                    1,
