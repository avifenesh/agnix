...
//!
//! Ensures all 132 rules from knowledge-base/rules.json are:
...
fn test_rules_json_integrity() {
    ...
    assert_eq!(
        rules_index.rules.len(),
        132,
        "Expected 132 rules in rules.json, found {}",
        rules_index.rules.len()
    );
...
fn test_sarif_rule_count() {
    ...
    // SARIF should have exactly 132 rules to match rules.json
    assert_eq!(
        sarif_rules.len(),
        132,
        "SARIF should have 132 rules, found {}. Missing or extra rules detected.",
        sarif_rules.len()
    );
}
