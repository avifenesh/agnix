...
//!
//! Ensures all 131 rules from knowledge-base/rules.json are:
...
fn test_rules_json_integrity() {
    ...
    assert_eq!(
        rules_index.rules.len(),
        131,
        "Expected 131 rules in rules.json, found {}",
        rules_index.rules.len()
    );
...
fn test_sarif_rule_count() {
    ...
    // SARIF should have exactly 131 rules to match rules.json
    assert_eq!(
        sarif_rules.len(),
        131,
        "SARIF should have 131 rules, found {}. Missing or extra rules detected.",
        sarif_rules.len()
    );
}
